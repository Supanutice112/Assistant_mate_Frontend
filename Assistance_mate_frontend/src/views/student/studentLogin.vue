<template>
    <div class="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-gray-100 to-gray-200 p-6">
      ...
      <div class="bg-white shadow-md rounded-lg p-6 w-full max-w-md mb-6 text-center">
        <h3 class="text-2xl font-semibold text-gray-700 mb-4">Student Login</h3>
  ...
  export default {
    ...
    methods: {
      async login() {
        try {
          const response = await axios.post('http://127.0.0.1:5000/api/student_login', {
          ...
          if (response.data.access_token) {
            ...
            this.$router.push('/studentdashboardView'); // Replace with the appropriate route
          } else {
            alert('Login failed');
          }
        } catch (error) {
